...
      const res = await api.get("/users/payment-info-requests/");
      // Luôn đảm bảo history là một mảng để tránh lỗi .find hoặc .some
      const results = res.data?.results || res.data || [];
      setHistory(Array.isArray(results) ? results : []);
      setHasPending(results.some((row) => row.status === "pending"));
    } catch {
      setHistory([]);
      setHasPending(false);
    }
...
      });
      setSnackbar({ open: true, message: "Yêu cầu đã được gửi thành công.", severity: "success" });
      setEditMode(false);
      fetchHistory(); // Gọi lại lịch sử để cập nhật giao diện
    } catch (err) {
...
  if (!profile) return <Alert severity="error">Không có dữ liệu người dùng.</Alert>;
  ...
  // ✅ BƯỚC 2: Chuẩn bị style để làm mờ thông tin nếu đã được duyệt.
...
        <Divider sx={{ mb: 2 }} />
        ...
        {/* ✅ BƯỚC 1: Hiển thị Alert khi isApproved là true */}
        {isApproved && (
...
        ) : (
          <Box>
            {/* ✅ BƯỚC 2: Áp dụng style làm mờ ở đây */}
            <Box sx={infoStyle}>
